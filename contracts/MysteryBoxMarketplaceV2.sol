...
contract MysteryBoxMarketplaceV2 is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, IERC721Receiver{
    ...
    address public _ecoRecepientAddress;
    
    uint256 private _buyBackFeePercent;       //Multipled by 1000
     ...
     */ 
    function getFeePercent() external view returns(uint256 buyBackFeePercent, uint256 ecoFeePercent){
        buyBackFeePercent = _buyBackFeePercent;
        ecoFeePercent = _ecoFeePercent;
    }
    
    function purchase(uint256 tokenId, uint256 requestPrice) payable external whenNotPaused nonReentrant returns(uint256){
        ...
        require(msg.value == tokenPrice, "Invalid request value price");

        uint256 ownerReceived = tokenPrice;
        uint256 feeAmount = 0;
        if(_buyBackFeePercent > 0){
            feeAmount = tokenPrice * _buyBackFeePercent / 100 / MULTIPLIER;
            if(feeAmount > 0){
                payable(address(_buyBackRecepientAddress)).transfer(feeAmount);
        ...
        if(_ecoFeePercent > 0){
            ...
            if(feeAmount > 0){
                payable(address(_ecoRecepientAddress)).transfer(feeAmount);
                ownerReceived -= feeAmount;
            }
        }
        payable(tokenOwner).transfer(ownerReceived);
    ...
    function setBuyBackRecepientAddress(address newAddress) external onlyOwner{
        require(newAddress != address(0), "Zero address");
        _buyBackRecepientAddress = newAddress;
    }

    function setEcoRecepientAddress(address newAddress) external onlyOwner{
        require(newAddress != address(0), "Zero address");
        _ecoRecepientAddress = newAddress;
    }
     ...
     */
    function setFeePercent(uint256 buyBackFeePercent, uint256 ecoFeePercent) external onlyOwner{
        require(buyBackFeePercent < 100 * MULTIPLIER, "Invalid burning fee percent");
        require(ecoFeePercent < 100 * MULTIPLIER, "Invalid ecosystem fee percent");
        _buyBackFeePercent = buyBackFeePercent;
